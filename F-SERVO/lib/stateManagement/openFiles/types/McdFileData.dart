...
class McdData extends _McdFilePart {
  ...
  static Future<String?> searchForTexFile(String initDir, String mcdName, String ext) async {
    // 1. Try directly in the provided directory (where the .mcd was extracted)
    String? texPath = join(initDir, mcdName + ext);
    if (await File(texPath).exists()) return texPath;

    // 2. If it's a .wtp and we are in a .dat extraction dir, try the .dtt peer
    if (initDir.endsWith(".dat") && ext == ".wtp") {
      var dttDir = "${initDir.substring(0, initDir.length - 4)}.dtt";
      var dttTexPath = join(dttDir, mcdName + ext);
      if (await File(dttTexPath).exists()) return dttTexPath;

      // Try to extract the .dtt if it exists but isn't extracted yet
    ...
    // 3. Fallback: search in the same directory as the .mcd but for common names
    // (In Wii U, title.wta/wtp might be in the same folder as messtitle.mcd)
    var parentDir = Directory(initDir);
    if (await parentDir.exists()) {
      await for (var entity in parentDir.list()) {
        if (entity is File && entity.path.toLowerCase().endsWith(ext.toLowerCase())) {
           // If we find any WTA/WTP in the same folder, use it as a candidate
           // for the MCD if the specific name match failed.
           return entity.path;
        }
      }
    }
  ...
  static Future<McdData> fromMcdFile(OpenFileId? file, String mcdPath) async {
    ...
    if (wtaPath == null) {
      showToast("Unable to find related .wta file!");
      throw Exception("Unable to find related .wta file!");
    }
    
    SizeInt textureSize = const SizeInt(1024, 1024); // Default fallback
    if (wtpPath != null) {
      try {
        textureSize = await getImageSize(wtpPath);
      }
      catch (e, s) {
        print("Warning: Unable to read size from ${basename(wtpPath)}, using fallback. $e");
      }
    }
