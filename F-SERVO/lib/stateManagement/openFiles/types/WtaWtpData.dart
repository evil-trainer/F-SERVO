...
class WtaWtpTextures with HasUuid, Undoable implements Disposable {
  ...
  static Future<WtaWtpTextures> fromWtaWtp(OpenFileId file, String wtaPath, String? wtpPath, String extractDir, bool isWtb) async {
    ...
    try {
      for (int i = 0; i < wta.textureOffsets.length; i++) {
        ...
        if (wta.isWiiU && !isWtb) {
          final wtpBytes = await File(wtpPath!).readAsBytes();
          textureBytes = makeWiiUGtx(wta, i, wtpBytes);
        } else {
          await wtpFile.setPosition(wta.textureOffsets[i]);
          textureBytes = await wtpFile.read(wta.textureSizes[i]);
        }
  ...
  Future<void> save() async {
    ...
    List<WiiUGtxPayload>? wiiUGtxPayloads;
    if (wta.isWiiU && !isWtb) {
      wiiUGtxPayloads = await Future.wait(textures.map((e) async {
        return parseWiiUGtx(await File(e.path.value).readAsBytes());
      }));
      wta.textureSizes = wiiUGtxPayloads.map((e) => e.dataLength).toList();
      wta.textureInfo = await Future.wait(textures.map((e) async {
        return await WtaFileTextureInfo.fromGtx(e.path.value);
      }));
      wta.textureMipmaps = List.filled(textures.length, 0);
    } else {
      // update sizes
    ...
    // update offsets (4096 byte alignment on PC, 0x2000 for Wii U GTX payloads)
    int offset = isWtb ? alignTo(wta.header.offsetTextureInfo + wta.textureInfo.length * wta.textureInfoRecordSize, 32) : 0;
    final textureAlignment = wta.isWiiU && !isWtb ? 0x2000 : 0x1000;
    for (int i = 0; i < wta.textureOffsets.length; i++) {
      offset = alignTo(offset, textureAlignment);
      wta.textureOffsets[i] = offset;
      offset += wta.textureSizes[i];
    }
    if (wiiUGtxPayloads != null) {
      offset = alignTo(offset, textureAlignment);
      for (int i = 0; i < wiiUGtxPayloads.length; i++) {
        final mipLength = wiiUGtxPayloads[i].mipmapLength;
        if (mipLength > 0) {
          offset = alignTo(offset, textureAlignment);
          wta.textureMipmaps![i] = offset;
          offset += mipLength;
        } else {
          wta.textureMipmaps![i] = 0;
        }
      }
    } else {
      offset = alignTo(offset, textureAlignment);
    ...
    try {
      for (int i = 0; i < wta.textureOffsets.length; i++) {
        await wtpFile.setPosition(wta.textureOffsets[i]);
        if (wiiUGtxPayloads != null) {
          await wtpFile.writeFrom(wiiUGtxPayloads[i].image);
        } else {
      ...
      if (wiiUGtxPayloads != null) {
        for (int i = 0; i < wta.textureOffsets.length; i++) {
          final mipOffset = wta.textureMipmaps![i];
          if (mipOffset != 0 && wiiUGtxPayloads[i].mipmaps.isNotEmpty) {
            await wtpFile.setPosition(mipOffset);
            await wtpFile.writeFrom(wiiUGtxPayloads[i].mipmaps);
          }
        }
      }
      var alignment = wta.isWiiU && !isWtb ? 0x2000 : 4096;
      var endPadding = await wtpFile.position() % alignment;
      if (endPadding != 0)
        await wtpFile.writeFrom(List.filled(alignment - endPadding, 0));
    } finally {
      await wtpFile.close();
    }
