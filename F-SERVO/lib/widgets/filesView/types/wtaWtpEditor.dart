...
class _TexturesTableConfig with CustomTableConfig {
  ...
  String get _textureExt => textures.any((e) => e.path.value.toLowerCase().endsWith(".gtx")) ? "gtx" : "dds";

  Future<void> _selectTexture(int index) async {
    final ext = _textureExt;
    var paths = await FilePicker.platform.pickFiles(
      dialogTitle: "Select ${ext.toUpperCase()}",
  ...
  Future<void> patchImportFolder() async {
    var folderSel = await FilePicker.platform.getDirectoryPath(
      dialogTitle: "Select folder with ${_textureExt.toUpperCase()} files",
    );
    if (folderSel == null)
      return;
    final ext = _textureExt;
    var allTextureFiles = await Directory(folderSel).list()
      .where((e) => e is File && e.path.toLowerCase().endsWith(".$ext"))
      .map((e) => e.path)
      .toList();
    List<String> ddsFilesOrdered = List.generate(allTextureFiles.length, (index) => "");
    for (var dds in allTextureFiles) {
    ...
    if (ddsFilesOrdered.any((e) => e.isEmpty)) {
      showToast("Some ${_textureExt.toUpperCase()} files are missing");
      return;
    }
    ...
    file.onUndoableEvent();
    showToast("Added ${ddsFilesOrdered.length} ${_textureExt.toUpperCase()} files");
  }
}
