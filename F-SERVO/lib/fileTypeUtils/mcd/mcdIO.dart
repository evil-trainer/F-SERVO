...
class McdFile {
  ...
  McdFile.fromParts(this.header, this.messages, this.symbols, this.glyphs, this.fonts,
      this.events, {this.endian = Endian.little});

  static bool _looksLikeValidHeader(ByteData data, int length, Endian endian) {
    int u32(int off) => data.getUint32(off, endian);
    final offsets = [u32(0), u32(8), u32(16), u32(24), u32(32)];
    final counts = [u32(4), u32(12), u32(20), u32(28), u32(36)];
    if (offsets.any((o) => o < 0x28 || o >= length))
      return false;
    if (counts.any((c) => c > 0x100000))
      return false;
    return true;
  }
  ...
  static Future<McdFile> fromFile(String path) async {
    ...
    final bigOk = _looksLikeValidHeader(data, bytes.length, Endian.big);
    final littleOk = _looksLikeValidHeader(data, bytes.length, Endian.little);
    bytes.endian = bigOk && !littleOk ? Endian.big : Endian.little;
    bytes.position = 0;
